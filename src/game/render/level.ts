...
export const drawBackground = (ctx: CanvasRenderingContext2D, cameraX: number, stage: number) => {
  ...
  throatGrad.addColorStop(0, palette.throatInner);
  throatGrad.addColorStop(1, palette.throatOuter);
  ...
  lightGrad.addColorStop(0.7, palette.light);
  ...
  ctx.fillStyle = '#fff';
  ctx.beginPath();
  ctx.ellipse(dentistX - 35, eyeY, 20, 15, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(dentistX + 35, eyeY, 20, 15, 0, 0, Math.PI * 2);
  ctx.fill();
  ...
  ctx.beginPath();
  ctx.arc(dentistX - 35, eyeY + 2, 8, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.arc(dentistX + 35, eyeY + 2, 8, 0, Math.PI * 2);
  ctx.fill();
  ...
  const toothColorTop = palette.toothTop;
  const toothColorBot = palette.toothBottom;
...
const drawRealisticTooth = (
  ...
  const stain = ctx.createLinearGradient(
    0,
    isTop ? ty : ty - h * 0.4,
    0,
    isTop ? ty + h * 0.4 : ty
...
export const drawTransition = (ctx: CanvasRenderingContext2D, progress: number, stage: number) => {
  ...
  const drawJaw = (yPos: number, isTop: boolean) => {
    ...
    gumGrad.addColorStop(0, '#831843');
    gumGrad.addColorStop(1, '#db2777');
