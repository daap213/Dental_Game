...
export const spawnProjectile = (projectiles: Projectile[], x: number, y: number, dx: number, dy: number, owner: 'player' | 'enemy', type: WeaponType | 'normal', player?: Player) => {
    ...
    if (owner === 'enemy') {
        projectiles.push({ ...base, x, y, w: 10, h: 10, vx: dx * 6, vy: 0, hp: 1, maxHp: 1, damage: 10, lifeTime: 2, projectileType: 'bullet', color: COLORS.projectileEnemy } as Projectile);
        return;
    }
    if (!player) return;
    ...
    if (type === 'spread') {
        ...
        for(let i=start; i<=end; i++) {
          ...
          projectiles.push({ ...base, x, y, w: width, h: width, vx: vx * 20, vy: vy * 20, hp: 1, maxHp: 1, damage: dmg, lifeTime: 0.8, projectileType: 'laser', color: COLORS.projectileLaser } as Projectile);
        ...
        if (level === 3) {
           ...
           projectiles.push({ ...base, x: x - (perpX*offset), y: y - (perpY*offset), w: 10, h: 6, vx: vx * 12, vy: vy * 12, hp: 1, maxHp: 1, damage: dmg, lifeTime: 1.0, projectileType: 'bullet', color: COLORS.projectilePlayer } as Projectile);
           projectiles.push({ ...base, x: x + (perpX*offset), y: y + (perpY*offset), w: 10, h: 6, vx: vx * 12, vy: vy * 12, hp: 1, maxHp: 1, damage: dmg, lifeTime: 1.0, projectileType: 'bullet', color: COLORS.projectilePlayer } as Projectile);
        } else {
           projectiles.push({ ...base, x, y, w: 10, h: 6, vx: vx * 12, vy: vy * 12, hp: 1, maxHp: 1, damage: dmg, lifeTime: 1.0, projectileType: 'bullet', color: COLORS.projectilePlayer } as Projectile);
        }
    }
};
